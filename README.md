📌 프로젝트 소개
Hanbit_front는 서울 여행객을 위한 여행지 추천 웹사이트입니다.
사용자의 관심사에 따라 맞춤형 여행지를 추천하고, 실시간 날씨 정보, 길찾기 기능, 여행지 밀집도 분석, 따릉이 대여소 정보 등을 제공합니다.
...
👥 팀원 및 역할
심하나: 프론트엔드 (길찾기, 네이버 지도 API 활용한 정보 제공, 따릉이 대여소 정보, 여행지 정보 불러오기, 밤/낮 모드 설정)
이하은: 프론트엔드 & 백엔드 (회원가입 및 로그인 기능, 네이버 API를 활용한 여행지 상세 정보 불러오기)
조하경: 프론트엔드 (서울 열린데이터 API를 활용한 데이터 분석 및 여행지 추천)
이창용: 프론트엔드 (여행지 팁 제공, 환율 정보 표시)

🚀 주요 기능
여행자의 관심사 기반 맞춤형 여행지 추천
네이버 지도 API를 활용한 길찾기 기능 제공
서울 열린데이터 광장 API를 활용한 여행지 밀집도 분석
따릉이 API를 활용한 대여소 정보 제공
여행지 API를 활용한 서울 명소 정보 제공
날씨 API를 활용한 실시간 기상 정보 제공
사용자 리뷰 및 평점 기능
...
VITE_NAVER_API_SECRET=your_api_secret
VITE_NAVER_MAP_CLIENT_ID=your_client_id
VITE_NAVER_MAP_SECRET=your_client_secret
VITE_KAKAO_REST_API_KEY=your_kakao_key
VITE_POPULATION_API_KEY=your_api_key
VITE_CITY_WEATHER_API_KEY=your_api_key
```
